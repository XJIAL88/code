...
class ComponentImage extends BaseComponent
{
	...
	static function cutAndZoom($filePath, $width, $height)
	{
		...
		try {
			...
			// ResultZ
//			self::logInfo(__CLASS__ . '->' . __FUNCTION__, 'savePath/' . $savePath, 'image');
			return $savePath;
		} catch (\Exception $e) {
			self::logError(__CLASS__ . '->' . __FUNCTION__, $e->getMessage(), 'image');
			self::logError(__CLASS__ . '->' . __FUNCTION__, 'filePath/' . $filePath, 'image');
			self::logError(__CLASS__ . '->' . __FUNCTION__, 'savePath/' . $savePath, 'image');
			return false;
		}
	}
