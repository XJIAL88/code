...
class ThumbHandler
{
    ...
    public $img_display_quality = 100;// 除png外的图片显示质量,范围1-100
    public $png_img_display_quality = 9;// png图片显示质量,范围1-9
    ...
    public $copy_w;// 拷贝图形宽
    public $copy_h;// 拷贝图形高
    public $src_x = 0;// 原图绘制起始横坐标
    public $src_y = 0;// 原图绘制起始纵坐标
...
//            fclose ($handle);
//        }
		$src = file_get_contents($src_img); // 现在PHP的版本不会低于4.3，所以一定会存在file_get_contents方法，因此注释上面代码
        if(empty($src))
        {
			// throw new C("图片源为空");
			throw new Exception("图片不存在"); // 此处代码报错，因此修改为下面的方式 2018-08-31 18:26
        }
    ...
    function setMaskFont($font=2)
    {
        ...
        {
            // throw new C("字体文件不存在");
			throw new Exception("字体文件不存在"); // 此处代码报错，因此修改为下面的方式 2018-08-31 18:26
        }
        $this->font = $font;
    }
...
//            fclose ($handle);
//        }
		$src = file_get_contents($this->mask_img); // 现在PHP的版本不会低于4.3，所以一定会存在file_get_contents方法，因此注释上面代码
    ...
    function _output()
    {
        ...
        if(function_exists($func_name))
        {
            ...
            $quality = $img_type=='png' ? $this->png_img_display_quality : $this->img_display_quality;
            $func_name($this->h_dst, $this->dst_img, $quality);
        }
        else
        {
            Return false;
